...
def main(image_files, output_folder, exposure_times,
    ...
    # perform colorspace enhancements on the best HDR image
    enhancedHDR_image = hdr.colorspaceEnhancement(bestHDR_image)
    cv2.imwrite(path.join(output_folder, "bestHDRColorEnhanced.png"), enhancedHDR_image)

    enhanced_histogram = hdr.computeHistogram(enhancedHDR_image.astype(np.uint8))
    enhanced_cdf = hdr.computeCumulativeDensity(enhanced_histogram)
    plotHistogramCDF(enhanced_histogram, enhanced_cdf, output_folder,
                     plot_filename="histCDF_bestHDRColorEnhanced")
    print("colorspace enhanced best HDR image complete")
...
if __name__ == "__main__":
    ...
    # required variables are set at beginning of main.py
    # set optional variables here as needed
    main(image_files,
         output_dir,
         EXPOSURE_TIMES,
         resize=True,
